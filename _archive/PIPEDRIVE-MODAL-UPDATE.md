# ROI Calculator - Updated Lead Modal with Pipedrive Form

## INSTRUCTIONS
Replace the "Lead Capture Modal" section in `/resources/roi-calculator.qmd` with this code.

Find this section (around line 350-450):
```html
<!-- Lead Capture Modal -->
```

Replace everything from `<!-- Lead Capture Modal -->` to the closing `:::` tag with the code below:

---

## NEW LEAD MODAL CODE (Copy from here)
...
Add this to the `<style>` section at the bottom of roi-calculator.qmd (inside the existing style tags):
...
1. ✅ `/resources/roi-calculator.qmd` - Lead modal section (replace as shown above)
2. ✅ `/js/roi-calculator.js` - Add Pipedrive integration code from `PIPEDRIVE-INTEGRATION-ADDON.js`

---

## WHAT THIS DOES

1. **Replaces** your custom form with Pipedrive's embedded form
2. **Keeps** your modal styling and branding
3. **Preserves** close button and footer
4. **Maintains** RODO compliance message
5. **Auto-fills** calculator data into Pipedrive (via JavaScript)
6. **Tracks** form submission in Google Analytics
7. **Shows** thank you modal after submission

---

## NEXT STEP

After updating the files, we'll test the integration!
